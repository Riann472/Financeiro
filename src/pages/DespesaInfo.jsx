                ...
                {despesa.gtin ? (
                    <div className="img">
                        {despesa.gtin.length == 44 || despesa.gtin.length == 47 || despesa.gtin.length == 48 ? (
                            ...
                            despesa.gtin.startsWith('000201') && despesa.gtin.length >= 150 ? (<img className="pix" src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${despesa.gtin}`} />) : (<h3>Codigo de barras invalido</h3>)
                        }</>)}
                    </div>
                ) : (
